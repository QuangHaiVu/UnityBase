...
public class DamageOnTouch : MonoBehaviour
{
    ...
    public enum CausedKnockbackDirections
    {
    ...
    public enum TakenKnockbackDirections
    {
    ...
    [Header("Damage Caused")] [Tooltip("The amount of health to remove from the player's health")]
    ...
    [Header("Feedback")] [Tooltip("the feedback to play when applying damage to a damageable")]
    ...
    protected UnitController _colliderUnitController;
    ...
    protected virtual void Awake()
    {
        ...
        _unitController = this.gameObject.GetComponent<UnitController>();
    ...
    public virtual void SetCorgiController(UnitController newController)
    {
        _unitController = newController;
    }
    ...
    protected virtual void OnCollideWithDamageable(Health health)
    {
        ...
        _colliderUnitController = health.gameObject.MMGetComponentNoAlloc<UnitController>();
    ...
    {
        if ((_colliderUnitController != null) && (DamageCausedKnockbackForce != Vector2.zero) &&
            (!_colliderHealth.Invulnerable))
        {
            ...
            {
                Vector2 totalVelocity = _colliderUnitController.Speed + _velocity;
            ...
            if (DamageCausedKnockbackDirection == CausedKnockbackDirections.BasedOnOwnerPosition)
            {
                ...
                Vector2 relativePosition = _colliderUnitController.transform.position - Owner.transform.position;
            ...
            {
                _colliderUnitController.SetForce(_knockbackForce);
            }

            if (DamageCausedKnockbackType == KnockbackStyles.AddForce)
            {
                _colliderUnitController.AddForce(_knockbackForce);
            }
        }
    }
    ...
    {
        if ((_unitController != null) && (DamageTakenKnockbackForce != Vector2.zero) && (!_health.Invulnerable))
        {
            _knockbackForce.x = DamageCausedKnockbackForce.x;
            if (DamageTakenKnockbackDirection == TakenKnockbackDirections.BasedOnSpeed)
            {
                Vector2 totalVelocity = _unitController.Speed + _velocity;
                _knockbackForce.x *= -1 * Mathf.Sign(totalVelocity.x);
            }

            if (DamageTakenKnockbackDirection == TakenKnockbackDirections.BasedOnDamagerPosition)
            {
                Vector2 relativePosition = _unitController.transform.position - _collidingCollider.bounds.center;
                _knockbackForce.x *= Mathf.Sign(relativePosition.x);
            }
            ...
            {
                _unitController.SetForce(_knockbackForce);
            }

            if (DamageTakenKnockbackType == KnockbackStyles.AddForce)
            {
                _unitController.AddForce(_knockbackForce);
            }
        }
    }
